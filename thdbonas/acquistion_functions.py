...
class AcquisitionFunctionType(Flag):
    EI = auto()
    MEI = auto() # multi expected improvement
...
def _multi_expected_improvement(mean: torch.Tensor,
                                sigma: torch.Tensor,
                                min_val: torch.Tensor):
    assert isinstance(mean, torch.Tensor), f'instance type error, {type(mean)}'
    assert isinstance(sigma, torch.Tensor), f'instance type error, {type(mean)}'
...
class AcquisitionFunction:
    def __init__(self, aftype: AcquisitionFunctionType = AcquisitionFunctionType.EI):
        if AcquisitionFunctionType.EI == aftype:
            self.af_func = _expected_improvement
        elif AcquisitionFunctionType.MEI == aftype:
            self.af_func = _multi_expected_improvement
        else:
